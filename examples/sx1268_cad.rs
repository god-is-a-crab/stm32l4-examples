                            ...
                            *state = State::Rx;
                            ...
                            if GET_IRQ_STATUS_BUFS.irq_status().cad_detected() {
                                ...
                                *state = State::Rx;
                                ...
                                if CAD_ATTEMPT < 1 {
                                    // first try no detection, try again
                                    CAD_ATTEMPT = CAD_ATTEMPT + 1;
                                } else {
                                    // second try, no detection
                                    CAD_ATTEMPT = 0;
...
fn main() -> ! {
    ...
    dp.EXTI.rtsr1().write(|w| w.tr1().set_bit().tr2().set_bit());
    dp.EXTI.imr1().write(|w| w.mr1().set_bit().mr2().set_bit());
