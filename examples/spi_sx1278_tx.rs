...
const R_FIFO: [u8; 5] = [0; 5];
const W_FIFO: [u8; 5] = [0b1000_0000, 0x01, 0x02, 0x03, 0x04];
const R_FIFO_ADDR_PTR: [u8; 2] = [0x0D, 0x00];
const W_FIFO_ADDR_PTR: [u8; 2] = [0b1000_0000 | 0x0D, 0x80];
const W_FIFO_ADDR_PTR1: [u8; 2] = [0b1000_0000 | 0x0D, 0x00];
const R_FIFO_RX_CURR_ADDR: [u8; 2] = [0x10, 0x00];
...
fn USART2() {
    ...
    if dp.USART2.isr().read().rxne().bit_is_set() {
        ...
        match received_byte {
            ...
            99 => {
                // c
                send_command(&W_FIFO_ADDR_PTR1, dp);
            }
            100 => {
                // d
